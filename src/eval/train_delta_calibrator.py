...
- Clips model/market probs into (1e-6, 1-1e-6) via shared helper before fitting
...
from src.model.market_relative_calibration import fit_delta_calibrator, save_delta_calibrator
from src.utils.odds_math import devig_home_prob, clip_prob
...
def _infer_model_prob_column(df: pd.DataFrame) -> str:
    ...
    for c in candidates:
        if c in df.columns:
            return c
    raise RuntimeError(f"[train_delta_calibrator] No model probability column found. Tried: {candidates}")
...
def _clip_prob_series(s: pd.Series) -> pd.Series:
    # Use shared strict helper elementwise to avoid silently “fixing” garbage.
    return pd.to_numeric(s, errors="coerce").apply(clip_prob)


def main() -> None:
    ap = argparse.ArgumentParser("train_delta_calibrator.py")
    ap.add_argument("--per_game", required=True, help="Path to per-game backtest CSV.")
    ap.add_argument("--out", required=True, help="Output path for saved calibrator (.joblib).")
    ap.add_argument("--min-samples", type=int, default=25)

    ap.add_argument("--train-start", default=None, help="Optional training start date (YYYY-MM-DD).")
    ap.add_argument("--train-end", default=None, help="Optional training end date (YYYY-MM-DD).")

    args = ap.parse_args()

    if not os.path.exists(args.per_game):
        raise FileNotFoundError(f"[train_delta_calibrator] per_game file not found: {args.per_game}")
    ...
    df = df.copy()
    df["model_prob_home_raw"] = _clip_prob_series(df[model_prob_col])
    ...
    df["market_prob_home"] = _clip_prob_series(df["market_prob_home"])
    ...
    # Drop anything still invalid after strict clipping/coercion
    df = df.dropna(subset=["model_prob_home_raw", "market_prob_home", "home_win_actual", "ml_home_consensus"]).copy()
    if df.empty:
        raise RuntimeError("[train_delta_calibrator] No valid rows after coercion/clipping")

    calibrator = fit_delta_calibrator(df, min_samples=int(args.min_samples))
    save_delta_calibrator(calibrator, args.out)

    n_buckets = len(calibrator.get("calibrators", {}))
    print(f"[train_delta_calibrator] Saved delta calibrator with {n_buckets} buckets to {args.out}")

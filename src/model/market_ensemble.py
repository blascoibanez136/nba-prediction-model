...
def _aggregate_from_normalized(df: pd.DataFrame) -> pd.DataFrame:
    ...
    spread_stats = (
        ...
        .reset_index()
        if not spreads.empty else
        pd.DataFrame(columns=["merge_key","home_spread_consensus","home_spread_dispersion"])
    )
    ...
    totals_stats = (
        ...
        .reset_index()
        if not totals.empty else
        pd.DataFrame(columns=["merge_key","total_consensus"])
    )
...
def _aggregate_from_wide(df: pd.DataFrame) -> pd.DataFrame:
    ...
    ml_home = _find(["ml_home","home_ml"])
    ml_away = _find(["ml_away","away_ml"])

    df = _safe_to_numeric(df, [spread_col, total_col, ml_home, ml_away])
    ...
    if ml_home and ml_away:
        ml = df[["merge_key",ml_home,ml_away]].copy()
        def _devig(r):
            ph = american_to_prob(r[ml_home])
            pa = american_to_prob(r[ml_away])
            return ph / (ph + pa) if ph and pa else np.nan
        ml["prob"] = ml.apply(_devig, axis=1)
        ml_stats = ml.groupby("merge_key")["prob"].mean().rename("home_ml_prob_consensus").reset_index()
    elif ml_home:
        ml = df[["merge_key",ml_home]].copy()
        ml["prob"] = ml[ml_home].apply(american_to_prob)
        ml_stats = ml.groupby("merge_key")["prob"].mean().rename("home_ml_prob_consensus").reset_index()
    else:
        ml_stats = pd.DataFrame(columns=["merge_key","home_ml_prob_consensus"])
...
def apply_market_ensemble(
    ...
    *,
    ...
    merged["market_weight"] = merged.home_spread_dispersion.apply(
        lambda d: compute_dispersion_weight(d,
    ...
    merged["fair_spread"] = merged.apply(
        lambda r:
            ...
            if pd.notna(r.fair_spread_market) else r.fair_spread_model,
        axis=1
    )
    ...
    merged["fair_total"] = merged.apply(
        lambda r:
            ...
            if pd.notna(r.fair_total_market) else r.fair_total_model,
        axis=1
    )

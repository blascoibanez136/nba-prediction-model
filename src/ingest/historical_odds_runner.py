...
using the historical endpoint and normalizes them using ``normalize_odds_list``.

For older seasons the odds payloads may be incomplete or structurally
different from the live format. In those cases the normalizer will
produce an empty DataFrame. This runner falls back to a wide builder
(``build_wide_snapshot_from_raw``) that extracts whatever information is
available so downstream market aggregation can still proceed.

Outputs:
    data/_snapshots/close_YYYYMMDD.csv    Normalized or wide snapshot
    data/_snapshots/raw/raw_YYYY-MM-DD.json  Raw Odds API response
"""

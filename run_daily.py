...
# -----------------------
# Helpers for odds snapshots
# -----------------------

def _latest_normalized_close_csv() -> Optional[str]:
    """
    Return the path to the latest normalized CLOSE odds CSV produced
    by src/ingest/odds_normalizer.py, or None if none exist.
    """
    snap_dir = SNAPSHOT_DIR
    if not snap_dir.exists():
        return None
    files = sorted(snap_dir.glob("close_*.csv"))
    if not files:
        return None
    return str(files[-1])
...
def build_model_predictions(games_df: pd.DataFrame, run_date: str) -> str:
    ...
    Adds a merge_key column so edge_picker can join on odds data reliably.
...
def apply_market_adjustment(preds_path: str, run_date: str) -> str:
    """
    Apply market-aware ensemble adjustment if a normalized CLOSE odds CSV
    is available. If not, fall back to model-only predictions.
    """
    ...
    close_csv = _latest_normalized_close_csv()
    if not close_csv:
        logger.info(
            "No normalized CLOSE odds CSV found in %s; using model-only predictions.",
            SNAPSHOT_DIR,
        )
        return preds_path

    logger.info("Using normalized CLOSE odds CSV for ensemble: %s", close_csv)
    odds_close = pd.read_csv(close_csv)

    out = apply_market_ensemble(preds, odds_close)
...
def main() -> None:
    ...
    # 5) Apply market ensemble adjustment (if normalized CLOSE odds are present)
